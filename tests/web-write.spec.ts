 ...
 * and the `save-concept` button) → the SvelteKit hook mints a JWT from the live
 * session → axum verifies it → `sunstone-server` write-then-commits into the
 * served fixture git repo. We assert the `save-concept` affordance, then a real
...
test('the `save-concept` button also persists an explicit commit', async ({ page }) => {
