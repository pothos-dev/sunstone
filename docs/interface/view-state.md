...
- `frontmatterShown` — the global Frontmatter show/hide toggle. (`propertiesShown`, its
  pre-[ADR 0008](/adr/0008-raw-yaml-frontmatter-editing.md) name, is still READ so an existing
...
- **Defaults on read.** The session store fills absent fields: the left Sidebar and most Sections default to **open**, but **Tags**, the **right Sidebar** and **Frontmatter** default to **collapsed/hidden**. `editorMode` defaults to `read`, and each Sidebar width defaults to `DEFAULT_SIDEBAR_WIDTH`.
