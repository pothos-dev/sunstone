...
- [ ] `render_concept` takes an asset-URL prefix and rewrites every Embed `src`, so the web view and the desktop PDF both show images
...
- **Transport** → a custom URI scheme on desktop, `/api/asset` on web, a URL (never bytes) across the seam, and an asset-URL prefix parameter on the shared renderer. See [ADR-0011](/adr/0011-attachment-bytes-cross-a-custom-uri-scheme.md).
...
- **Attachment index** → a **separate** index from `all_paths`, not a type-tagged single index. `find_bundle_root` infers the Bundle root structurally from `.md` paths, and a top-level `assets/logo.png` folded into that input could shift the inferred root and break every bundle-absolute link in the Bundle. A separate index makes the `.md`-only guarantee hold by construction rather than by remembering a filter.
