...
| `src/api_error.rs` | The HTTP error types (`ApiError`, `WriteError`), the read/write status classifiers and the `guard_rel_path` network-boundary check. |
...
| `src/write.rs` | Write orchestration: composes sunstone-native writers + a git commit per op (none in the plain shape), decides amend-vs-fresh-commit, and produces the SSE change groups to broadcast. |
| `src/testutil.rs` | Test-only helpers: collision-free temp dirs, a seeded Bundle, a `ServerState` builder, and the git CLI harness. |

## HTTP routes

All under `/api`. Reads are open GETs; writes require a verified JWT and return `204 No Content` unless noted.

**Reads:** `GET /api/bundle-root`, `/api/tree` (`TreeNode`), `/api/concept?path=` (raw markdown), `/api/render?path=` (`RenderPayload`), `/api/search?q=`, `/api/backlinks?path=`, `/api/tags`, `/api/concepts-by-tag?tag=`, `/api/types`, `/api/keys`, `/api/concept-paths`, `/api/attachment-paths` (every Attachment path in the index, sorted — a separate list from `concept-paths`, the Embed resolver's candidate set), `GET /api/asset?path=` (an Attachment's raw bytes with a `Content-Type` from the shared mime table; the path is guarded syntactically and then resolved against the canonical root, so an in-Bundle symlink pointing outside is a 400 too), and `GET /api/events` — the **SSE** stream (`text/event-stream`) carrying `FileChange` events for live reload plus, under the named `sync` event, the two [sync notices](#the-sync-loop-git-synced-only).
