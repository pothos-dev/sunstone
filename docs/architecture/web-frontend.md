...
- **`src/lib/web/`** — the web-specific layer: `WebViewer` (SSR read-only shell) and its sidebar pieces; `WebAppShellIsland` (authenticated users dynamically `import()` the full `App.svelte`, keeping CodeMirror out of SSR, and host the write-concurrency coordinator — this full App shell is the only web write path; the former in-place `WebEditorIsland` editor has been removed); `WebConcurrencyModals` (shared conflict / leave / structural-op UX); plus pure helpers `loadConcept.ts`, `conceptUrl.ts`, `concurrency.ts`, `uiState.ts`, `urlSync.ts` (URL ⇄ active-Concept reconciliation), `syncNotice.ts` (sync-loop notice copy), `webViewerUiState.ts` (WebViewer's persisted-UI snapshot/restore), `webMermaid.ts` (client-side Diagram hydration of server HTML) and `remoteEmbed.ts` (click-to-load for remote Embeds). The viewer's sidebar pieces (`WebTree`, `WebTags`, `WebOutline`, `WebBacklinks`, `WebSearch`, `WebQuickNav`) are deliberately separate from the desktop components: they take no dependency on the desktop `focus` / `session` stores (editor-only infrastructure the read surface has no need for), have no dnd / CRUD, and keep their open / expanded / keyboard state locally.
...
- **`src/lib/state/`** — Svelte 5 runes state singletons (`.svelte.ts`): `editor`, `workspace`, `document`, `bundle`, `index` (owns the `BundleIndex` wasm handle), `session` (per-Bundle restorable UI state, persisted through the seam), `theme`, `treeActions`, `focus` (the Region focus backbone), and the keyboard-nav stores `explorerNav`, `tagsNav`, `listFocusNav` (bound to its rendered list by `listRegionNav`); plus pure plain-`.ts` helpers `layoutPersist.ts` (tiling-layout serialize/validate), `navHistory.ts` (per-Tile Back/Forward) and `sessionState.ts` (the `BundleState` ⇄ session-field mapping, defaults and migrations).
- **`src/lib/components/`** — desktop components (`Tile`, `TileHeader`, `Tree`, `ExplorerPane`, `ActivityRail`, `Sidebar`, `SidebarEdge`, `Outline`, `Backlinks`, `Frontmatter`, `SearchPanel`, `QuickNav`, `Launcher`, …) — the [interface](/interface/index.md) surfaces.
- **Pure logic modules** — plain `.ts` with unit tests beside them, per the repo convention that pure logic stays testable: `path.ts`, `treeNav.ts`, `fuzzy.ts`, `highlight.ts`, `launcherRows.ts`, `reserved.ts`, and `frontmatter.ts`'s **YAML text helpers** (ADR-0008 `joinConcept` / `isParseable` / `titleFromYaml` / `formatYaml` / `scaffoldConcept`, kept TS by design — they ride the `yaml` package's CST, which `serde_yaml` cannot provide). The link, slug, outline, citation, anchor-rewrite and frontmatter-parse twins that used to live here were **deleted** — that logic is now [sunstone-shared](/architecture/sunstone-shared.md) via the wasm seam, so the two sides can no longer drift ([ADR 0006](/adr/0006-wasm-shared-core-for-frontend-logic.md)).
