//! Pure bundle-path helpers (ADR 0006 §2): the '/'-separated, bundle-relative
//! path math shared by markdown-link resolution, the index, the rewrite engine,
//! and the wasm handle.
//!
//! This is the PURE subset of the former `sunstone-native::paths`: the
//! `ignore`-backed `bundle_walker` / `md_files` stay native (they need the
//! filesystem); everything here is a pure transform, exhaustively unit-testable
//! and `wasm32`-safe.
...
/// the `SCHEME_RE` in the former `src/lib/links.ts`.
...
/// dropped (matching the backend's escape rejection and the former `links.ts`).
