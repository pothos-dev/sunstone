...
# af-1: Embedded images in a Concept
...
- **Remote images** → click-to-load, never automatic. Opening a Concept must not fire a tracking pixel from a reader's browser. `data:` URIs do not render at all: they are an injection vector once [af-2](af-2-theme-aware-svg-embeds.md) inlines SVG, and they carry no benefit a Bundle file does not.
